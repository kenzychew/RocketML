...
- `demo-railway/static/index.html` carries the same portfolio-hub palette and font stack (Fraunces/Public Sans/JetBrains Mono) as `demo/main.py`, as plain CSS custom properties in a `<style>` block — there is no separate `style.css`; the page has always kept its styling inline, and that stays the pattern for future edits.
